...
Original file is located at
    https://colab.research.google.com/drive/1jYtdrSevzHOax2lVJWGTy_VIV0yBPN9G
"""
...
df = pd.read_csv("/content/Dataset_sate.csv")

X = df[['sate', 'weekend', 'awal_bulan', 'akhir_bulan']]
y = df['y']
...
new_data = pd.DataFrame({'sate': [250], 'weekend': [1], 'awal_bulan': [0], 'akhir_bulan': [1]})
...
print(f"Predicted optimal stock item (nasi): {prediction[0]}")

new_data = pd.DataFrame({'sate': [226], 'weekend': [0], 'awal_bulan': [0], 'akhir_bulan': [0]})
new_data_normalized = (new_data - X_train_mean) / X_train_std
prediction_scaled = model.predict(new_data_normalized).flatten()
prediction = prediction_scaled * y.std() + y.mean()
print(f"Predicted optimal stock item (sate): {prediction[0]}")

import pickle

model_info = (model, X_train_mean, X_train_std, y.mean(), y.std())
with open('/content/modelsate.pkl', 'wb') as file:

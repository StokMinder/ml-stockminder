...
Original file is located at
    https://colab.research.google.com/drive/1OrKXBV6xKaHHrTwuIwXEVgaDBrEk6q1X
"""
...
df = pd.read_csv("/content/Dataset_nasi.csv")

X = df[['nasi', 'weekend', 'awal_bulan', 'akhir_bulan']]
y = df['y']
...
new_data = pd.DataFrame({'nasi': [250], 'weekend': [1], 'awal_bulan': [0], 'akhir_bulan': [1]})
...
new_data = pd.DataFrame({'nasi': [199], 'weekend': [0], 'awal_bulan': [0], 'akhir_bulan': [0]})
new_data_normalized = (new_data - X_train_mean) / X_train_std
prediction_scaled = model.predict(new_data_normalized).flatten()
prediction = prediction_scaled * y.std() + y.mean()
print(f"Predicted optimal stock item (nasi): {prediction[0]}")
